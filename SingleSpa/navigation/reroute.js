...
export const reroute = () => {
  ...
  function performAppChange() {
    ...
    // 1. 卸载应用
    const unmountPrmoise = Promise.all(appsToUnmount.map(toUnmountPromise));
    // 2. 加载应用(可能这个应用在注册的时候就已经被加载了)
    const mountPromise =Promise.all(appsToLoad.map(app=>toLoadPromise(app).then(app=>{
      // 当应用加载1完毕后，需要启动和挂载，但是要保证挂载前，先卸载应用
      tryBootstrapAndMount(app, unmountPrmoise);
    })));
    function tryBootstrapAndMount(app, unmountPrmoise) {
      if(shouldBeActive(app)) {
        // 保证卸载完毕再挂载
        return toBootstrapPromise(app).then(app=>unmountPrmoise.then(()=>toMountPromise(app)))
      }
    }
  }
}